...
char INTCON = 0x00;
char T1CON = 0x01;
char ADCON1 = 0x07; // Todos los pines como digitales 
/*
	bTRISA.AsChar = DefaultTRISA;
	bTRISB.AsChar = DefaultTRISB;
	bTRISC.AsChar = DefaultTRISC;
	bTRISD.AsChar = DefaultTRISD;
	bTRISE.AsChar = DefaultTRISE;

	bCopyPORTA.AsChar = DefaultPORTA;
	bPORTA = bCopyPORTA;
	bCopyPORTB.AsChar = DefaultPORTB;
	bPORTB = bCopyPORTB;
	bCopyPORTC.AsChar = DefaultPORTC;
	bPORTC = bCopyPORTC;
	bCopyPORTD.AsChar = DefaultPORTD;
	bPORTD = bCopyPORTD;
	bCopyPORTE.AsChar = DefaultPORTE;
	bPORTE = bCopyPORTE;
*/
...
/************************************
 LCD
 ************************************/
#define LCD_STROBE()			{ E_PIN = true; displayPort = bCopyPORT; Nop(); Nop(); E_PIN = false; DATA_PORT = bCopyPORT; }
#define LCD_STROBE_READ(value)	{ E_PIN = true; displayPort = bCopyPORT; Nop(); Nop(); value = displayPort; E_PIN = false; displayPort = bCopyPORT; }
#define LCD_NIBBLE(value)		{ displayPort = value; displayPort = bCopyPORTD; LCD_STROBE(); }
byte bCopyPORT;
unsigned char lcd_read_cmd_nowait(void)
{
	unsigned char c, readc;
	DATA_PORT = 0x0F;
	bCopyPORTA.AsBits.RW = true;			// Read LCD
	bPORTA = bCopyPORTA;
	...
	bCopyPORTA.AsBits.RW = false;	// Return to default mode of writing LCD
	bPORTA = bCopyPORTA;			
	bTRISD.AsBits.Data = 0x00;		// Return to default mode of writing LCD
	return(c);
}
...
{
	lcd_check_busy(); // Check LCD busy flag
	bCopyPORTA.AsBits.RS = true;
	bPORTA = bCopyPORTA;
	LCD_NIBBLE( ( c >> 4 ) & 0x0F );
	LCD_NIBBLE( c & 0x0F );
	bCopyPORTA.AsBits.RS = false;
	bPORTA = bCopyPORTA;
}
...
void UserDisplayInit(byte i){
	...
	mInitDisplay(); /*inicio Display en TRISA*/
	lcd_init();
	lcd_clear();
}//end UserDisplayInit
...
void UserDisplayRelease(byte i){
	...
	mReleaseDisplay(); /*libero el TRISA*/
}
